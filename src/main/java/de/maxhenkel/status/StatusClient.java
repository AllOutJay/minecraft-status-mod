...
public class StatusClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        CLIENT_CONFIG = ConfigBuilder
                ...
                .path(Minecraft.getInstance().gameDirectory.toPath().resolve("config").resolve(Status.MODID).resolve("state-client.properties"))
                .build();
        ...
        KEY_STATUS_GUI = KeyMappingHelper.registerKeyMapping(new KeyMapping("key.status_gui", InputConstants.KEY_U, KeyMapping.Category.MISC));

        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (KEY_STATUS_GUI.consumeClick()) {
                client.gui.setScreen(new StatusScreen());
            }
        });
    }
}
