...
Adds an optional feature that greets players logging in with a summary of which currently-online players have set an availability status (e.g. _Do Not Disturb_ / _Open_).

### Changes

- **New server config option `announce_statuses_on_login`** (default `false`). When enabled, each player joining the server receives a chat message listing other online players grouped by availability.
- **`Availability` enum extended** with a translation key and a `ChatFormatting` color per value:
  - `OPEN` → green
  - `DO_NOT_DISTURB` → red
  - `NONE` → white
- **`PlayerStateManager.announceStatuses(...)`** sends the grouped summary on join. Each availability group is rendered as a colored `■` marker, a translatable label, and a comma-separated, case-insensitively sorted list of player names.

### Example chat output:

⬜ No Availability: Ash298
🟩 Open: AllOutJay, KashMuney
🟥 Do Not Disturb: Blondskunk, MineyLO

### Behavior notes

- Disabled by default, no change for existing servers
- Only sent to clients that have the mod installed
- The joining player is excluded from the lists, and empty groups are skipped.
